...
param.T = 1.0;                          % can be chosen larger 
param.delta_T = 0.033;
param.N = ceil(param.T/param.delta_T);  % can be chosen smaller
param.tau = linspace(0,param.T,param.N);
param.t = 8;
param.MPCsteps = ceil(param.t/param.delta_T);
param.fixPointIter = 20;
param.damping = 0.75;
...
for k = 1:param.MPCsteps
      ...
      u = [h1(x(1,:)',lambda(1,:)',param);h2(x(1,:)',lambda(1,:)',param)];
