...
%% draw
    for i = 1:num_agents
        ...
        for j = 1:param.sens_iter_k-1
            cur_agent.collect_result = [cur_agent.collect_result, max(cur_agent.collect_first(:,j*3+1:j*3+3) - cur_agent.collect_last(:,(j-1)*3+1:(j-1)*3+3))];
        end

        matrix_1 = cur_agent.collect_result(:, 1:3:end);
        matrix_2 = cur_agent.collect_result(:, 2:3:end);
        matrix_3 = cur_agent.collect_result(:, 3:3:end);
        
        figure; 
        plot(1:param.sens_iter_k-1, matrix_1, '-o');
        hold on;

        plot(1:param.sens_iter_k-1, matrix_2, '-x');

        plot(1:param.sens_iter_k-1, matrix_3, '-*');
    
        title(['Evaluation ', num2str(i), ' xi']);
        xlabel('Iteration k');
        ylabel('$$\|\delta x_i^k\|_{L_\infty}$$', 'Interpreter', 'latex');
        legend('||\delta x_1^k_{L_\infty}||','||\delta x_2^k_{L_\infty}||','||\delta x_3^k_{L_\infty}||');
        hold off;  
    end
    ...
    for i = 1:num_agents
        ...
        xlabel('Time');
        ylabel('Sensitivity state Variables');
        legend('x_1', 'x_2','x_3');
    end
