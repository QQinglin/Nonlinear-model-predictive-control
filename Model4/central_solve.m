function [u_k,lambda_k, x_k] = central_solve(opt_param, model)
    ...
    %% draw
    data_u_q = [t_span]'; % *****************
    data_u_q = [data_u_q,x_k];
    writematrix(data_u_q, 'C:\Users\24966\Desktop\project\4Cen_x_q.csv'); % *********************

    figure;
    plot(t_span, x_k(:, 1), '-x', t_span, x_k(:, 2), '-x',t_span, x_k(:, 3), '-x');
    ...
    data_lambda_q = [t_span]'; % *****************
    data_lambda_q = [data_lambda_q,lambda_k];
    writematrix(data_lambda_q, 'C:\Users\24966\Desktop\project\4Cen_lambda_q.csv'); % *********************

    figure;
    plot(t_span, lambda_k(:, 1), '-o', t_span, lambda_k(:, 2), '-o',t_span, lambda_k(:, 3), '-o');
    ...
    data_u_q = [t_span]'; % *****************
    data_u_q = [data_u_q,u_k];
    writematrix(data_u_q, 'C:\Users\24966\Desktop\project\4Cen_u_q.csv'); % *********************

    figure;
    plot(t_span, u_k(:, 1), '-x', t_span, u_k(:, 2), '-x',t_span, u_k(:, 3), '-x');
