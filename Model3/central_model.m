classdef central_model < handle
    ...
    methods
        ...
        function x_dot = ffct(obj, x, u)
            x_dot = [-x(1)^3 + u(1)*x(1) + sin(x(2) - x(1));
                -2* x(2)^3 + u(2)*x(2) + cos(x(1)-x(2)) + sin(x(3) - x(2));
                -3* x(3) ^3 + u(3)*x(3) + sin(x(2) - x(3))];
        end

        % Jacobian matrix of dynamics
        function f_dot = dfdx(obj, x, u)
            f_dot = [-3*x(1)^2+u(1)-cos(x(2)-x(1)), cos(x(2)-x(1)), 0;
                     -sin(x(1)-x(2)), -6*x(2)^2 + u(2) + sin(x(1)-x(2)) - cos(x(3)-x(2)), cos(x(3)-x(2));
                     0, cos(x(2)-x(3)), -9*x(3)^2 + u(3) - cos(x(2) - x(3))];
        end
