function  solve_centralOCP(central,opt_param)
            % solves the OCP via the fixed point iteration method
            x0 = central.x0; % x0 = [1;1;1]
            lambda_q = central.lambda0; % 21 x 3 ones
            t_span = opt_param.t;
            T_end = t_span(end);
            %% Begin of Algorithm
            for q=1:opt_param.grad_iter
                % forward integration of the state dynamics
                x_q = ode4(@(t,x)central.dH_dlambda(t,x,lambda_q,t_span),t_span,x0);

                % backward integration of the adjoint state dynamics
                rev_t_span = flip(t_span);
                x_T = x_q(end,:);             % 1x3
                lambda_T = central.dVdx(x_T); % 3x1
                lambda_q_rev = ode4(@(t,lambda)central.dH_dx(t,x_q,lambda,t_span),rev_t_span,lambda_T);
                lambda_q = flip(lambda_q_rev);
            end

            % distribute states, controls adjoint states to agents 
            % update the output
            u_q = central.calcu_uq(x_q,lambda_q); 
